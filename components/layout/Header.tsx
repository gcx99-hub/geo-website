...
export default function Header() {
  ...
  return (
    <header className="sticky top-0 z-50 bg-deep-blue/80 backdrop-blur-sm text-white border-b border-white/10">
      <div className="max-w-content mx-auto px-4 md:px-6 py-3 flex justify-between items-center">
      ...
      {isOpen && (
        ...
        >
          <nav className="flex flex-col gap-4">
            <Link href="#timeline" onClick={() => setIsOpen(false)} className="text-white/80">周期</Link>
            <Link href="#critique" onClick={() => setIsOpen(false)} className="text-white/80">观察</Link>
            <Link href="#cta" onClick={() => setIsOpen(false)} className="text-white/80">联系</Link>
          </nav>
        </motion.div>
      )}
    </header>
